...
recipes.addImage = async(input) => {
    ...
    return new Promise(async(resolve) => {
        try {
            ...
            let newPendingSubmission = {
                img_url: `https://imagedelivery.net/IFvEIogeVzND8ViLE4CLUw/${input.destination}`,
                is_pending: true,
                success: true,
...
recipes.updateImage = async(input, idx, userId) => {
    ...
    return new Promise(async(resolve) => {
        try {
            ...
            await db.Recipe.findOneAndUpdate({_id: idx}, {img_url: `https://imagedelivery.net/IFvEIogeVzND8ViLE4CLUw/${input.destination}`});
